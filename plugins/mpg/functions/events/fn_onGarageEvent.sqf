...
switch (_event) do {
	case "Load": {
		...
		{
			_ctrlMouseArea ctrlAddEventHandler [_x, format [QUOTE((['MouseArea.%1'] + _this) call FUNC(onGarageEvent);), _x]];
		} forEach ["MouseButtonDown", "MouseButtonUp", "MouseMoving", "MouseZChanged"];

		["UpdateCamera"] call FUNC(onGarageEvent);

		{
			private _ctrl = _display displayCtrl _x;
			if (ctrlEnabled _ctrl) exitWith {
				[_display, _ctrl, true] call FUNC(onLeftTabSelect);
			};
		} forEach [IDC_MPG_TABCAR, IDC_MPG_TABTANK, IDC_MPG_TABHELICOPTER, IDC_MPG_TABPLANE, IDC_MPG_TABNAVAL, IDC_MPG_TABSTATIC];
		...
		_fnc_initMenu = {
			...
			if (_size == 0) then {
				...
				if (_menuData != "") then {
					_control menuSetAction [_path, format [QUOTE(_this + ['%1'] call FUNC(onMenuBar);), _menuData]];
				};
			} else {
