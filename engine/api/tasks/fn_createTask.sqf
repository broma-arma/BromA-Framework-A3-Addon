...
if (_details isEqualType "") then {
	private _cfgTaskDescription = [["CfgTaskDescriptions", [_details, _id] select (_details == "")], configfile >> ""] call BIS_fnc_loadClass;
...
if (_position isEqualType [] && { _position select 0 isEqualType objNull }) then {
	...
	if (!isNull _object) then {
		if (_delay isEqualType true) then {
			...
			_positionUpdate = [_object, compile (if (_delay isEqualType []) then {
...
BRM_FMK_Engine_tasks set [_id, [
	...
	[
		...
		format ["(%1) ", _priorityName select [0, 1]] + _title // Title
	], _type, _position, if (isNil "_positionUpdate") then { nil } else { _positionUpdate }, 0,
