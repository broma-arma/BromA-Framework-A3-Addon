...
USAGE:
	...
	["HIGH_TIER", true] call BRM_FMK_Engine_fnc_getLoadoutCode;
...
BRM_FMK_Engine_fnc_getLoadoutCodeCache getOrDefaultCall [_key, {
	...
	if (_structure && isArray _cfg) then { // Note: Don't use `exitWith`
		[getArray _cfg select 0, _structure] call BRM_FMK_Engine_fnc_getLoadoutCode
	} else {
		private _parentCode = "";
		private _file = if (isArray _cfg) then {
			getArray _cfg params ["_parentID", "_file"];
			_parentCode = format ["call ([""%1"", false] call BRM_FMK_Engine_fnc_getLoadoutCode);%2", _parentID, toString [10]];
			_file
		} else {
			getText _cfg
		};
...
%4", _key, _file, "#", _parentCode]]
			...
			if (_id != "DEFAULT") then {
				...
				["DEFAULT", _structure] call BRM_FMK_Engine_fnc_getLoadoutCode
			} else {
