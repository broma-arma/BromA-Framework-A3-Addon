...
plank_deploy_fnc_createFortification = {
    ...
    _displayName = GET_FORT_DISPLAY_NAME((_unit getVariable "plank_deploy_fortIndex"));
    _duration = GET_FORT_DURATION((_unit getVariable "plank_deploy_fortIndex"));
    _distance = GET_FORT_DISTANCE((_unit getVariable "plank_deploy_fortIndex"));

    _removeAction = ["DeleteObject", "Remove", "", {

        _args = _this select 2;
        _duration = _args select 0;
        _unit = _args select 1;
        _displayName = _args select 2;
    ...
    } , {(vehicle player == player)}, {}, [_duration, _unit, _displayName], [0,0,0], _distance] call ACE_interact_menu_fnc_createAction;
    [_fort, 0, ["ACE_MainActions"], _removeAction] call ACE_interact_menu_fnc_addActionToObject;
...
plank_deploy_fnc_addPlacementActions = {
    ...
    _confirmActionId = _unit addAction ['<t color="#3748E3">Confirm Deployment</t>', BRM_FMK_Plugin_PlankBuilding_fnc_confirmFortAction, [], 100, false, false, "", BRM_FMK_PlankBuilding_deployCondition];
    _cancelActionId = _unit addAction ['<t color="#FF0000">Cancel Deployment</t>', BRM_FMK_Plugin_PlankBuilding_fnc_cancelFortAction, [], 99, false, false, "", "driver _target == _this"];
    _openActionId = _unit addAction ['<t color="#00FF00">Open Settings</t>', BRM_FMK_Plugin_PlankBuilding_fnc_openSettingsAction, [], 98, false, false, "", "driver _target == _this"];
...
plank_deploy_fnc_confirmFortPlacement = {
    ...
    [
        ...
        { player playAction "medicStop" },
        format ["Building %1...", _displayName]
    ] call ace_common_fnc_progressBar;
};
...
plank_deploy_fnc_addFortificationAction = {
    ...
    if (_count > 0 && {_unit getVariable ["plank_deploy_fortIndex", -1] != _fortIndex}) then {
        ...
        _actionId = _unit addAction [format ["Place %1 (%2 left)", GET_FORT_DISPLAY_NAME(_fortIndex), _count], BRM_FMK_Plugin_PlankBuilding_fnc_placeFortAction, [_fortIndex], _fortIndex + 50, false, false, "", BRM_FMK_PlankBuilding_deployCondition];
        _fortActionIds = _unit getVariable ["plank_deploy_fortActionIds", []];
