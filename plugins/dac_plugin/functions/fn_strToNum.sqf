...
NAME:
    BRM_FMK_Plugin_DAC_fnc_strToNum
...
USAGE:
    ["SKILL", "decent"] call BRM_FMK_Plugin_DAC_fnc_strToNum;
...
switch (_stat) do {
	case "SIDE": {
		switch (_value) do {
			...
			case "friendly";
			case "side_a": { [_stat, side_a_side] call BRM_FMK_Plugin_DAC_fnc_strToNum };
			case "enemy";
			case "side_b": { [_stat, side_b_side] call BRM_FMK_Plugin_DAC_fnc_strToNum };
			case "neutral";
			case "side_c": { [_stat, side_c_side] call BRM_FMK_Plugin_DAC_fnc_strToNum };
			default { ["[WARN ] [DAC Plugin] Unknown side '%1', defaulting to 'enemy'", _value] call BIS_fnc_error; [_stat, side_b_side] call BRM_FMK_Plugin_DAC_fnc_strToNum };
		};
	};
	...
	case "FACTION": {
		...
		if (_index == 0) then {
			...
			if (_index isEqualType sideUnknown) then {
				_index = [_stat, [_index, "FACTION"] call BRM_FMK_fnc_getSideInfo] call BRM_FMK_Plugin_DAC_fnc_strToNum;
			};
		};
