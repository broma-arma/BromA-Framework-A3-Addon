...
if (isServer) then {
	["BRM_FMK_taskStateChanged", {
		...
		if (_state == "SUCCEEDED") then {
			[missionNamespace getVariable format ["side_%1_side", _sideChar], BRM_FMK_Plugin_RespawnSystem_respawnObjective] call BRM_FMK_RespawnSystem_fnc_callRespawnSide;
		}
