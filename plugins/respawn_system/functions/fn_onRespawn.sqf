...
if (((player_current_lives) == 0) || !(mission_allow_respawn)) then {
    ...
    [player, 0] call BRM_FMK_RespawnSystem_fnc_setLives;
    ...
    [player] spawn BRM_FMK_RespawnSystem_fnc_removeFromMission;
} else {
    player setVariable ["unit_deaths", (player getVariable ["unit_deaths",0])+1,true];
    [player, (player_current_lives-1)] call BRM_FMK_RespawnSystem_fnc_setLives;
