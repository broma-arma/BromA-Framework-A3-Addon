...
0 spawn {
	...
	waitUntil {!isNull findDisplay 46};
        ...
        (findDisplay 46) displayAddEventHandler ["KeyDown","_this call BRM_FMK_CHVD_fnc_keyIsPressed"];

	//Detect when to change setting type
	[] spawn {
		for "_i" from 0 to 1 step 0 do {
			[nil, false] call BRM_FMK_CHVD_fnc_updateSettings;
	...
	[] spawn {
		for "_i" from 0 to 1 step 0 do {
			...
			[nil, true] call BRM_FMK_CHVD_fnc_updateSettings;
			waitUntil {UAVControl (getConnectedUAV player) select 1 == ""};
			[nil, false] call BRM_FMK_CHVD_fnc_updateSettings;
		};
	};
