...
switch (_event) do {
	...
	case "Unload": {
		...
		if (_exitCode == 1/*OK*/) then {
			if (_save) then {
				["SaveVehicle", ctrlText (_dialog displayCtrl IDC_MPG_SAVES_NAME)] call FUNC(onSavesEvent);
			} else {
				private _ctrlList = _dialog displayCtrl IDC_MPG_SAVES_LIST;
				...
				if (_index != -1) then {
					...
					if (_saveIndex >= 0) then {
						["LoadVehicle", _saveIndex] call FUNC(onSavesEvent);
					};
				};
			};
		};
	...
	case "LoadVehicle": {
		...
		[_display, _display displayCtrl (IDCS_LEFT select _vehicleTypeIndex), false] call FUNC(onLeftTabSelect);
