...
fnc_civFiredWeapon = {
    ...
    if !(_unit in hostile_civilians) then {
        ...
        ["SERVER", "F_LOG", format ["SERVER: %1 IS NOW CONSIDERED HOSTILE",_unit]] call BRM_FMK_fnc_doLog;
    };
};
        ...
        if ((isPlayer _killer) && (_sideKiller in responsible_sides)) then {
            ...
            switch (true) do {
                case (_sideKiller == side_a_side): {
                    ...
                    ["CLIENTS", "HINT", format ["%3 CIVILIAN CASUALTIES: %1 out of %2", dead_civilians_side_a, mission_dead_civilian_limit, side_a_name]] call BRM_FMK_fnc_doLog;
                };
                case (_sideKiller == side_b_side): {
                    dead_civilians_side_b = dead_civilians_side_b + 1; publicVariable "dead_civilians_side_b";
                    ["CLIENTS", "HINT", format ["%3 CIVILIAN CASUALTIES: %1 out of %2", dead_civilians_side_b, mission_dead_civilian_limit, side_b_name]] call BRM_FMK_fnc_doLog;
                };
                case (_sideKiller == side_c_side): {
                    dead_civilians_side_c = dead_civilians_side_c + 1; publicVariable "dead_civilians_side_c";
                    ["CLIENTS", "HINT", format ["%3 CIVILIAN CASUALTIES: %1 out of %2", dead_civilians_side_c, mission_dead_civilian_limit, side_c_name]] call BRM_FMK_fnc_doLog;
                };
            };
            dead_civilians = dead_civilians + 1; publicvariable "dead_civilians";
            ["SERVER", "F_LOG", format ["SERVER: %2 HAS KILLED CIVILIAN %1", name _killed, name _killer]] call BRM_FMK_fnc_doLog;
        };
    };
