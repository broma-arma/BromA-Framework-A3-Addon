    ...
    if ((count tasks_a > 0)&&(mission_running)) then {
        _count = 0;
        {
            ...
            if (_wincond) then {
                ...
                [_id,"succeeded", true] call BRM_FMK_fnc_setTask;
                if !([_id] in tasks_done_a) then {
                    tasks_done_a pushBack [_id];

                    if ("respawn_system" in usedPlugins) then {
                        [side_a_side, mission_respawn_objective] spawn BRM_FMK_RespawnSystem_fnc_callRespawnSide;
                    };
            ...
            if (_losecond) then {
                if (_primary || _secondary) then {
                    [_id,"failed", true] call BRM_FMK_fnc_setTask;
                    if (_primary) then {
                    ...
                    [_id,"canceled", true] call BRM_FMK_fnc_setTask;
                };
            };
        } forEach tasks_a;
        ...
        if ((_count == (count tasks_primary_a)) && (count tasks_primary_a >= 1)) then {
            ...
            if (side_a_side in mission_require_extraction && "f_evade_escape" in usedPlugins) then {
                if (!a_extracting) then {
                    [extraction_a select 0, extraction_a select 1, 100, "mission_require_extraction = mission_require_extraction - [side_a_side]"] spawn BRM_FMK_f_evade_escape_fnc_reachObject;

                    [side_a_side,"aExtract",["Extract","Make your way to an extraction zone.", "exit", []],
        ...
        _count = 0;
        {
            ...
            if (_wincond) then {
                ...
                if !([_id] in tasks_done_b) then {
                    ...
                    if ("respawn_system" in usedPlugins) then {
                        [side_b_side, mission_respawn_objective] spawn BRM_FMK_RespawnSystem_fnc_callRespawnSide;
                    };
        ...
        if ((_count == (count tasks_primary_b)) && (count tasks_primary_b >= 1)) then {
            ...
            if (side_b_side in mission_require_extraction && "f_evade_escape" in usedPlugins) then {
                if (!b_extracting) then {
                    [extraction_b select 0, extraction_b select 1, 100, "mission_require_extraction = mission_require_extraction - [side_b_side]"] spawn BRM_FMK_f_evade_escape_fnc_reachObject;

                    [side_b_side,"bExtract",["Extract","Make your way to an extraction zone.", "exit", []],
    ...
    if ((count tasks_c > 0)&&(mission_running)) then {
        _count = 0;
        {
            ...
            if (_wincond) then {
                ...
                if !([_id] in tasks_done_c) then {
                    ...
                    if ("respawn_system" in usedPlugins) then {
                        [side_c_side, mission_respawn_objective] spawn BRM_FMK_RespawnSystem_fnc_callRespawnSide;
                    };
        ...
        if ((_count == (count tasks_primary_c)) && (count tasks_primary_c >= 1)) then {
            ...
            if (side_c_side in mission_require_extraction && "f_evade_escape" in usedPlugins) then {
                if (!b_extracting) then {
                    [extraction_c select 0, extraction_c select 1, 100, "mission_require_extraction = mission_require_extraction - [side_c_side]"] spawn BRM_FMK_f_evade_escape_fnc_reachObject;

                    [side_c_side,"bExtract",["Extract","Make your way to an extraction zone.", "exit", []],
