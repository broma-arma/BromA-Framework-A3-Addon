...
deployChair = _unit addaction ["<t color='#0099FF'>Put chair down</t>",{ player spawn BRM_FMK_ChairPlugin_fnc_chairDeploy; }, _unit];

_localChair = _unitChair createVehicleLocal (position player);
_deployed = false;

while {(_unit call BRM_FMK_ChairPlugin_fnc_canHoldChair) && (_unitChair != "nothing")} do {
...
if (_unitChair != "nothing") then {
    player spawn BRM_FMK_ChairPlugin_fnc_chairDeploy;
};