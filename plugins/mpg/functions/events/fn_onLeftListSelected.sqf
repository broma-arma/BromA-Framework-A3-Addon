...
private _vehiclePylons = [_vehicle] call FUNC(getPylons);
...
// Create GUI controls for vehicle pylons.
{
	...
	_ctrlPylon ctrlAddEventHandler ["ButtonClick", {
		...
		[ctrlParent _control, 2, true, _pylonIndex] call FUNC(loadRightContent);
	}];
...
[_display, _ctrlRightTab, false] call FUNC(onRightTabSelect);
