...
if (hasInterface) then {
	[{(!isNil "co_lock_allSidesReady") && (!isNil "locked_sides")}, {
		...
		private _coLockTextSelect = parseNumber BRM_FMK_Plugin_CommanderLock_tvtMode;
		...
		if (player in BRM_FMK_Plugin_CommanderLock_units) then {
			...
			private _coLockActionCondition = "(player in BRM_FMK_Plugin_CommanderLock_units)&&(side player in locked_sides)";
			...
			player addAction [_addActionText, {
				...
				private _coLockNotificationParameters = if (BRM_FMK_Plugin_CommanderLock_tvtMode) then {
			...
			private _coLockPlayerFreeCondition = (((!BRM_FMK_Plugin_CommanderLock_tvtMode) && (side player in locked_sides)) || ((BRM_FMK_Plugin_CommanderLock_tvtMode) && (!co_lock_allSidesReady)));
			...
			if (_coLockPlayerFreeCondition) then {
				...
				[{!(((!BRM_FMK_Plugin_CommanderLock_tvtMode) && (side player in locked_sides)) || ((BRM_FMK_Plugin_CommanderLock_tvtMode) && (!co_lock_allSidesReady)))}, {
