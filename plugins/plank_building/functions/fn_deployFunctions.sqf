...
plank_deploy_fnc_addPlacementActions = {
    ...
    _confirmActionId = _unit addAction ['<t color="#3748E3">Confirm Deployment</t>', FUNC(confirmFortAction), [], 100, false, false, "", GVAR(deployCondition)];
    _cancelActionId = _unit addAction ['<t color="#FF0000">Cancel Deployment</t>', FUNC(cancelFortAction), [], 99, false, false, "", "driver _target == _this"];
    _openActionId = _unit addAction ['<t color="#00FF00">Open Settings</t>', FUNC(openSettingsAction), [], 98, false, false, "", "driver _target == _this"];
...
plank_deploy_fnc_addFortificationAction = {
    ...
    if (_count > 0 && {_unit getVariable ["plank_deploy_fortIndex", -1] != _fortIndex}) then {
        ...
        _actionId = _unit addAction [format ["Place %1 (%2 left)", GET_FORT_DISPLAY_NAME(_fortIndex), _count], FUNC(placeFortAction), [_fortIndex], _fortIndex + 50, false, false, "", GVAR(deployCondition)];
        _fortActionIds = _unit getVariable ["plank_deploy_fortActionIds", []];
